...
local function CreatePlayerButton(player, index)
    ...
    PlayerButton.Text = player.Name -- Mostrar el nombre original en la lista
    ...
    -- Inicializar el estado del botón como pausado
    buttonStates[player.Name] = "paused"

    PlayerButton.MouseButton1Click:Connect(function()
        if buttonStates[player.Name] == "paused" then
            -- Si está pausado, iniciar el bucle de teletransporte
            buttonStates[player.Name] = "teleporting"
            while buttonStates[player.Name] == "teleporting" do
                TeleportPlayer(player.Name)
                wait(0.001)  -- Esperar un segundo antes de teletransportarse nuevamente
            end
        else
            -- Si está teletransportando, pausar el bucle de teletransporte
            buttonStates[player.Name] = "paused"
        end
    end)
end
