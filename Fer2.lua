...
local function transform()
    ...
    while not lplr.Status:FindFirstChild("Transformation") and isLoop2Active do task.wait() end
    if not Boss then
    end
    ...
    elseif side.Value == "Evil" and isLoop2Active then
        forms = {
          {"Astral Instinct",120e6,"Blanco"},	
            {"Beast",120e6,"Blanco"},
    ...
    if planet == "Earth" and ldata.Rebirth.Value >= 20000 then
        if getloweststat() < 30e6 and isLoop2Active then return end
        local useform = nil
        for i,form in pairs(forms) do
            if form[2] == 30e6 and isLoop2Active then useform = form[1] break end
        end
        while lplr.Status.Transformation.Value ~= useform and isLoop2Active do
            game:GetService("ReplicatedStorage").Package.Events.equipskill:InvokeServer(useform)
            if lplr.Status.Transformation.Value == useform and isLoop2Active then return end
            pcall(function()
    ...
    if FindChar() then
        if getloweststat() < 1e12 then -- (ldata.Rebirth.Value*3e6)+2e6
            ...
            for i,form in pairs(forms) do
                if currentform == form[1] or (form[3] and currentform == form[3]) and isLoop2Active then return end
                if getloweststat() >= form[2] and isLoop2Active then 
                    
                    game:GetService("ReplicatedStorage").Package.Events.equipskill:InvokeServer(form[1])
                    if form[3] ~= nil and isLoop2Active then
...
task.spawn(function() -- Move/Attack
    while ScGui do
        if Farming and isLoop4Active then
            if _G.Key ~= r then
                return
            end
            task.spawn(function() 
            	pcall(function()
	                ...
	                if Boss and lplr.Character.Humanoid.Health > 0 and Boss.Humanoid.Health > 0 then
	                    if CanAttack and not Stacking then
	                        ...
	                        task.spawn(function()
                                ...
                                if getloweststat() >= 40000 and ldata.Quest.Value ~= "" and not lplr.Status:FindFirstChild("Invincible")  and isLoop3Active then
                                    ...
                                    for i,move in pairs(moves) do
                                        if not lplr.Status:FindFirstChild(move)  and isLoop3Active then
                                            spawn(function()
                                    ...
                                    if not attacked  and isLoop3Active then
                    ...
                    elseif table.find(mobs,Boss.Name) and isLoop3Active then
                        game:GetService("ReplicatedStorage").Package.Events.p:FireServer("Blacknwhite27",1)
                        game:GetService("ReplicatedStorage").Package.Events.p:FireServer("Blacknwhite27",2)
...
local allowedPlayers = {
    ...
    "andygamer012345", "NifraO7", "fernando_snake", "R4T4TOPP0",
    "Gotenks_129", "juancarlosvillo"
