...
local quests = {
  ...
  { name = "X Fighter Trainer", nickname = "X Fighter", requiredValue = 0, endRange = 30000 },
    ...
    { name = "Top X Fighter", nickname = "X Fighter Master", requiredValue = 300001, endRange = 700000 },
    { name = "Super Vegetable", nickname = "Super Vegetable", requiredValue = 750001, endRange = 1200000 },
    { name = "Perfect Atom", nickname = "Perfect Atom", requiredValue = 1200000, endRange = 5000000 },
    { name = "SSJ2 Wukong", nickname = "SSJ2 Wukong", requiredValue = 5000001, endRange = 8000000 },
    ...
    { name = "Vis (20%)", nickname = "Vis (20%)", requiredValue = 1200000001, endRange = 1800000000 },
    { name = "Vegetable (LBSSJ4)", nickname = "Vegetable (LBSSJ4)", requiredValue = 1800000001, endRange = 2700000000 },
    { name = "Wukong (LBSSJ4)", nickname = "Wukong (LBSSJ4)", requiredValue = 2700000001, endRange = 4200000000 },
    { name = "Vekuta (LBSSJ4)", nickname = "Vekuta (LBSSJ4)", requiredValue = 4200000001, endRange = 5000000000 },
    { name = "Wukong Rose", nickname = "Wukong Rose", requiredValue = 5000000001, endRange = 5500000000 },
    { name = "Vekuta (SSJBUI)", nickname = "Vekuta (SSJBUI)", requiredValue = 5500000001, endRange = 2000000000000000000 },
}

local function tpTo(boss)
...
local function assignMission()
    local strength = data.Strength.Value

    -- Verificar si el jugador tiene los rebirths y fuerza necesarios para la misión "Mapa"
    if data.Rebirth.Value > 2000 and strength <= 300000 then
        return "Mapa", "Mapa" -- Asigna la misión "Mapa" si cumple con los requisitos
    end

    -- Verificar si está en el rango de fuerza para "SSJG Kakata" y fuera de un lugar específico
    if strength > 200000000 and data.Zeni.Value < 15000 and game.placeId ~= 5151400895 then
        return "SSJG Kakata", "SSJG Kakata"
    end

    -- Asignar misión según la fuerza si no cumple con el requisito para "Mapa"
    for _, quest in ipairs(quests) do
        if strength >= quest.requiredValue and strength <= quest.endRange then
            return quest.name, quest.nickname
        end
    end
end
...
task.spawn(function()
    while true do
        task.wait() -- Ajustar el tiempo de espera para evitar sobrecargar el servidor
        local mission, bossName = assignMission()

        -- Buscar todos los jefes vivos con el mismo nombre
        local bosses = game:GetService("Workspace").Living:GetChildren()
        ...
        for _, boss in ipairs(bosses) do
            if boss.Name == bossName and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 then
                tpTo(boss)
                bossFound = true
                break -- Teletransportarse al primer jefe vivo encontrado
            end
        end

        -- Si no se encontró un jefe vivo, puedes implementar la lógica necesaria aquí

        -- Verificar si la misión actual es diferente y actuar en consecuencia
        if data.Quest.Value ~= mission then
            local npc = game:GetService("Workspace").Others.NPCs:FindFirstChild(mission)
            if npc then
